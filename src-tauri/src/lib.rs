...
fn validate_signed_url(url: &str) -> Result<(), AppError> {
    if url.starts_with("https://") || url.starts_with("http://") {
        Ok(())
    } else {
        Err(AppError::Validation(
            "Signed URL must be an HTTP(S) URL.".to_string(),
        ))
    }
}
...
fn install_server_runtime() -> Result<PathBuf, AppError> {
    ...
    let output = Command::new(uv_path)
        ...
        .map_err(|e| {
            AppError::Spawn(format!(
                "Failed to run uv tool install --upgrade kenkui: {e}"
            ))
        })?;
...
fn ensure_server_runtime() -> Result<PathBuf, AppError> {
    find_server_runtime().map_or_else(install_server_runtime, Ok)
}

fn server_command() -> Result<Command, AppError> {
...
#[cfg(unix)]
fn signal_process_group(signal: &str, process_group_id: u32) -> Result<(), AppError> {
    let status = Command::new("/bin/kill")
        .arg(format!("-{signal}"))
        .arg(format!("-{process_group_id}"))
        .status()
        .map_err(|e| AppError::Process(format!("Failed to signal kenkui process group: {e}")))?;
    if status.success() {
        Ok(())
    } else {
...
fn tail_file(path: &Path, max_bytes: u64) -> Result<Vec<String>, AppError> {
    let mut file = File::open(path)
        .map_err(|e| AppError::Io(format!("Failed to open {}: {e}", path.display())))?;
    let len = file
        .metadata()
        .map_err(|e| AppError::Io(format!("Failed to inspect {}: {e}", path.display())))?
        .len();
    let start = len.saturating_sub(max_bytes);
    file.seek(SeekFrom::Start(start))
        .map_err(|e| AppError::Io(format!("Failed to seek {}: {e}", path.display())))?;
...
fn output_folder_for_path(path: &Path) -> Result<PathBuf, AppError> {
        ...
        path.parent()
            .ok_or_else(|| {
                AppError::Runtime(format!(
                    "Could not determine output folder for {}",
                    path.display()
                ))
            })?
            .to_path_buf()
...
#[tauri::command]
async fn open_output_folder(path: String) -> Result<(), AppError> {
    let folder = output_folder_for_path(Path::new(&path))?;
    open_folder(&folder)
}

#[tauri::command]
async fn file_stat(path: String) -> Result<NativeFileStat, AppError> {
    let path_buf = PathBuf::from(&path);
    let metadata = std::fs::metadata(&path_buf)
        .map_err(|e| AppError::Io(format!("Failed to inspect file metadata: {e}")))?;
    if !metadata.is_file() {
        return Err(AppError::Validation("Selected path is not a file.".to_string()));
    }
    let filename = path_buf
        .file_name()
        .and_then(|value| value.to_str())
        .ok_or_else(|| AppError::Validation("Selected file has no valid filename.".to_string()))?
        .to_string();
    Ok(NativeFileStat {
        path,
        filename,
...
#[tauri::command]
async fn file_sha256(path: String) -> Result<String, AppError> {
    let bytes = std::fs::read(&path)
        .map_err(|e| AppError::Io(format!("Failed to read file for sha256: {e}")))?;
    let digest = Sha256::digest(&bytes);
    Ok(hex::encode(digest))
}

#[tauri::command]
async fn signed_upload_file(
    path: String,
    url: String,
    content_type: String,
) -> Result<(), AppError> {
    ...
    let bytes = std::fs::read(&path)
        .map_err(|e| AppError::Io(format!("Failed to read upload file: {e}")))?;
...
#[tauri::command]
async fn signed_download_file(url: String, output_path: String) -> Result<(), AppError> {
    validate_signed_url(&url)?;
    let response = reqwest::Client::new()
        .get(&url)
        .send()
    ...
    let bytes = response
        .bytes()
        .await
        .map_err(|e| AppError::Http(format!("Failed to read signed download response: {e}")))?;
    let output = PathBuf::from(output_path);
    if let Some(parent) = output.parent() {
        std::fs::create_dir_all(parent)
            .map_err(|e| AppError::Io(format!("Failed to create download folder: {e}")))?;
    }
    std::fs::write(&output, bytes)
        .map_err(|e| AppError::Io(format!("Failed to write signed download: {e}")))
}
...
fn try_wait_for_exit(server: &mut ManagedServer) -> Result<bool, AppError> {
    server
        .child
        .try_wait()
        .map(|status| status.is_some())
        .map_err(|e| AppError::Process(format!("Failed to inspect kenkui process: {e}")))
}

fn request_graceful_shutdown(server: &ManagedServer) -> Result<(), AppError> {
    #[cfg(unix)]
    {
...
fn force_shutdown(server: &mut ManagedServer) -> Result<(), AppError> {
    #[cfg(unix)]
    {
    ...
    #[cfg(not(unix))]
    {
        server
            .child
            .kill()
            .map_err(|e| AppError::Process(format!("Failed to kill kenkui process: {e}")))
    }
}
...
fn shutdown_child(server: &mut ManagedServer) -> Result<(), AppError> {
...
fn shutdown_server_process(state: &ServerProcess) -> Result<(), AppError> {
...
#[tauri::command]
async fn spawn_server(app: AppHandle, state: State<'_, ServerProcess>) -> Result<(), AppError> {
    let mut lock = state.child.lock().unwrap();
    ...
    let mut child = command
        ...
        .map_err(|e| {
            ...
            *state.last_error.lock().unwrap() = Some(message.clone());
            AppError::Spawn(message)
        })?;

    let stdout = child
        .stdout
        .take()
        .ok_or_else(|| AppError::Spawn("Failed to capture kenkui stdout".to_string()))?;

    let stderr = child
        .stderr
        .take()
        .ok_or_else(|| AppError::Spawn("Failed to capture kenkui stderr".to_string()))?;
...
#[tauri::command]
async fn kill_server(state: State<'_, ServerProcess>) -> Result<(), AppError> {
    shutdown_server_process(&state)
}

#[tauri::command]
async fn server_logs(state: State<'_, ServerProcess>) -> Result<Vec<String>, AppError> {
    Ok(combined_log_tail(&state.logs))
}

#[tauri::command]
async fn server_status(state: State<'_, ServerProcess>) -> Result<ServerStatus, AppError> {
    let (running, pid) = {
        let mut lock = state.child.lock().unwrap();
...
#[cfg(target_os = "macos")]
fn security_output(args: &[&str]) -> Result<std::process::Output, AppError> {
    Command::new("/usr/bin/security")
        .args(args)
        .output()
        .map_err(|e| AppError::Auth(format!("Failed to access macOS Keychain: {e}")))
}

#[cfg(target_os = "macos")]
fn write_auth_session(session_json: &str) -> Result<(), AppError> {
...
#[cfg(target_os = "macos")]
fn read_auth_session() -> Result<Option<String>, AppError> {
    let output = security_output(&[
...
#[cfg(target_os = "macos")]
fn delete_auth_session() -> Result<(), AppError> {
    let output = security_output(&[
        "delete-generic-password",
        "-s",
        AUTH_SERVICE,
...
fn write_auth_session(_session_json: &str) -> Result<(), AppError> {
    Err(AppError::Auth(
        "Secure auth storage is not implemented on this platform yet.".to_string(),
    ))
}
...
#[tauri::command]
async fn clear_auth_session() -> Result<(), AppError> {
    delete_auth_session()
}

fn read_callback_target(stream: &mut TcpStream) -> Result<String, AppError> {
    stream
        .set_read_timeout(Some(Duration::from_secs(5)))
        .map_err(|e| {
            AppError::Auth(format!("Failed to configure auth callback socket: {e}"))
        })?;
    let mut buffer = [0; 8192];
    let read = stream
        .read(&mut buffer)
        .map_err(|e| AppError::Auth(format!("Failed to read auth callback request: {e}")))?;
    let request = String::from_utf8_lossy(&buffer[..read]);
    let request_line = request
        .lines()
        .next()
        .ok_or_else(|| AppError::Auth("Auth callback request was empty.".to_string()))?;
    let mut parts = request_line.split_whitespace();
    let method = parts.next().unwrap_or_default();
    let target = parts.next().unwrap_or_default();
    if method != "GET" || !target.starts_with("/auth/callback") {
        return Err(AppError::Auth(
            "Auth callback request did not target /auth/callback.".to_string(),
        ));
    }
    Ok(target.to_string())
}
...
#[tauri::command]
async fn start_auth_callback_listener(app: AppHandle) -> Result<String, AppError> {
    let listener = TcpListener::bind(("127.0.0.1", 0))
        .map_err(|e| AppError::Auth(format!("Failed to bind auth callback listener: {e}")))?;
    let addr = listener
        .local_addr()
        .map_err(|e| {
            AppError::Auth(format!(
                "Failed to read auth callback listener address: {e}"
            ))
        })?;
    listener.set_nonblocking(true).map_err(|e| {
        AppError::Auth(format!(
            "Failed to configure auth callback listener: {e}"
        ))
    })?;
