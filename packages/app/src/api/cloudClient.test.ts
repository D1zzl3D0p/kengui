...
import { CloudApiError, cloudFunctionsUrl, cloudRequest, redactSignedUrls } from './cloudClient';
...
describe('cloud client', () => {
  it('does not retain an arbitrary cloud response body in its error', async () => {
    const secret = 'Bearer PROVIDER_SECRET /Users/alice/private.epub';
    fetchMock.mockResolvedValueOnce(new Response(secret, { status: 500 }));

    const error = await cloudRequest('cancel-job').catch((value: unknown) => value);

    expect(error).toBeInstanceOf(CloudApiError);
    expect(error).toMatchObject({ status: 500 });
    expect((error as Error).message).toBe('Kengui Cloud request failed.');
    expect(JSON.stringify(error)).not.toContain(secret);
  });

  it('preserves canonical client-generated validation copy without accepting suffixes', () => {
    const safe = 'Sign in to Kengui Cloud before submitting cloud jobs.';
    expect(new CloudApiError(401, safe).message).toBe(safe);
    expect(new CloudApiError(401, `${safe} Bearer SPOOFED_SECRET`).message)
      .toBe('Kengui Cloud request failed.');
  });
